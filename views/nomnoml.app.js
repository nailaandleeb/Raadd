...
$(function (){
	...
	nomnoml.objextract=function(){
		$("#canvas").show();
		 $("#diagram").hide();
		 $.get("http://localhost:1234/usecase_diagrams",function(usecases){
		  ...
		  anchor.addEventListener("click", function(){ 
			$.get("http://localhost:1234/extract/"+usecases[a].id,function(res){
				editor.setValue(res);
				console.log(res)
				$('#myModal').modal('hide');
			});
	...
	nomnoml.clsextract=function(){
		$("#canvas").show();
		 $("#diagram").hide();
		var idz={};
		$.get("http://localhost:1234/object_diagrams",function(objects){
		$.get("http://localhost:1234/sequence_diagrams",function(seqs){
		...
		if(!document.getElementsByClassName("modal-bodytwo")[0].innerText){
			$.get("http://localhost:1234/extractClass/"+idz.obj_id+"/"+idz.seq_id,function(resp){
				//console.log(resp);
				if(resp.message){
					$('#ModalTwo').modal('hide');
					var d="[empty]";
	...
	nomnoml.seqextract=function(){
		...
		$.get("http://localhost:1234/object_diagrams",function(objects){
		 ...
		 // console.log(objects[u].id);
			$.get("http://localhost:1234/extractSeq/"+objects[u].id,function(res){
